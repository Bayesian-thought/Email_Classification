...
dictionary_words = file('dictionary.txt').read().split()
# remove extraneous characters
for i in range(len(dictionary_words)):
    dictionary_words[i] = dictionary_words[i].strip("'-")
    dictionary_words[i] = dictionary_words[i].replace("\'", "")
dictionary = set(dictionary_words)
...
def suggest(word):
    ...
    if word not in suggestion_cache:
        suggestion_cache[word] = correct(word)
    return suggestion_cache[word]
